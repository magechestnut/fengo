...
class Chestnut_Blog_Adminhtml_PostController extends Mage_Adminhtml_Controller_Action 
{
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            if(isset($_FILES['image']['name']) and (file_exists($_FILES['image']['tmp_name']))) {
                try {
                    ...
                    $path = Mage::getBaseDir('media') . DS . 'chestnut' . DS . 'blog' . DS;
                               
                    $uploader->save($path, $_FILES['image']['name']);

                    $data['image'] = 'chestnut/blog/' . preg_replace('/[^a-z0-9_\\-\\.]+/i', '_',$_FILES['image']['name']);
                }catch(Exception $e) {
