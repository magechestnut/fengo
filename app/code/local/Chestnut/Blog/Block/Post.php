...
class Chestnut_Blog_Block_Post extends Chestnut_Blog_Block_Abstract
{
    ...
    public function getRelatedHtml($post)
    {
        if ($post->getRelated()) {
            return $this->setTemplate('chestnut/blog/related.phtml')->setPost($post)->renderView();
        }
    }
    ...
    {
        Mage::helper('blog/toolbar')->create($this, 
            array(
        ...
        if ($breadcrumbs) {
            ...
            if ($title) {
                $breadcrumbs->addCrumb('category', array(
                    ...
                    'link' => Mage::getUrl('blog', array('category' => $this->getCategory()->getIdentifier())))
                );
            }
    ...
    protected function getCategory()
    {
        if (!$this->hasData('postCategory')) {
            $this->setData('postCategory', Mage::getSingleton('blog/category')->load($this->getRequest()->getParam('cat'), "identifier"));
        }
