...
class Chestnut_Blog_Block_Adminhtml_Post_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form
{
    protected function _prepareForm() {
        ...
        ))->setRenderer($this->getLayout()->createBlock('blog/adminhtml_post_renderer_identifier'));
        ...
        $categories = array();
        $collection = Mage::getModel('blog/category')->getCollection()->setOrder('sort_order', 'asc');
        ...
        try {
            ...
            $config->setData(Mage::helper('blog')->recursiveReplace(
                            '/blog/', '/' . (string) Mage::app()->getConfig()->getNode('admin/routers/adminhtml/args/frontName') . '/', $config->getData()
                    )
            );
        } catch (Exception $ex) {
            $config = null;
        ...
        $fieldset->addField('related', 'text', array(
            'name' => 'related',
            'label' => Mage::helper('blog')->__('Related Posts'),
            'title' => Mage::helper('blog')->__('Related Posts'),
            'style' => 'width:700px;',
            'after_element_html' => Mage::helper('blog')->__('Input post ids with space or comma as separators'),
        ));
