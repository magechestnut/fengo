...
abstract class Chestnut_Blog_Block_Abstract extends Mage_Core_Block_Template
{
    ...
    protected static $_catUriParam = Chestnut_Blog_Helper_Data::CATEGORY_URI_PARAM;
    
    protected static $_postUriParam = Chestnut_Blog_Helper_Data::POST_URI_PARAM;
    
    protected static $_tagUriParam = Chestnut_Blog_Helper_Data::TAG_URI_PARAM;
    ...
    protected function _processCollection($collection, $category = false)
    {
        ...
        foreach ($collection as $item) {           
            ...
            if ($shortContent) {
                ...
                $strManager = new Chestnut_Blog_Helper_Substring(array('input' => self::$_helper->filterWYS($item->getPostContent())));
    ...
    {
        return self::$_helper->getConfig($path, $store);
    }

    public function getBookmarkHtml($post)
    {
        if ($this->getConfig(Chestnut_Blog_Helper_Data::XML_BOOKMARKS)) {
            return $this->setTemplate('chestnut/blog/bookmark.phtml')->setPost($post)->renderView();
        }
    }
    ...
    public function getTagsHtml($post)
    {
        if (trim($post->getTags())) {
            return $this->setTemplate('chestnut/blog/line_tags.phtml')->setPost($post)->renderView();
        }
    }

    public function getCategoryLinks($postId)
    {
        $links = '<div class="postCategories">';
        $index = 0;
        $categoryCollection = Mage::getModel('blog/category')->getCollection()->addPostFilter($postId);
        foreach ($categoryCollection as $category) {
            if ($index++ > 0) 
                $links .= ', ';
            $links .= '<a href="' . $this->getBlogUrl($category->getIdentifier()) . '">';
            $links .= $category->getTitle();
            $links .= '</a>';
        }
        $links .= '&nbsp;&middot;&nbsp;</div>';
    ...
    public function getTagHref($tag = null)
    {
        $route = Mage::helper('blog')->getRoute();

        if (is_string($tag)) {
            return Mage::getUrl($route . "/tag/view/name/" . urlencode($tag));
        } elseif ($tag instanceof Chestnut_Blog_Model_Tag) {
            return Mage::getUrl($route . "/tag/view/name/" . urlencode($tag->getTag()));
        }
    ...
    public function getBlogUrl($key = null)
    {
        if (is_null($key)) {
            $blogUri = self::$_helper->getRoute();
        } else {
            $blogUri = self::$_helper->getRoute() . "/{$key}.html";
        }        
        return trim($this->getUrl($blogUri), "/");
    }
    ...
    {        
        $this->_helper('blog/toolbar')->create($this, 
            array(
    ...
    {      
        return '<a class="chestnut-blog-read-more" href="'.$item->getAddress().'">'.$this->__('Read More').' <em class="fa fa-angle-right"></em></a>';
    }
        ...
        if (self::$_helper->isEnabled()) {
            ...
            if($this->getParentBlock()) {
                if ($type == self::LINK_TYPE_HEADER) {
                    ...
                    $this->getParentBlock()->addLink($title, self::$_helper->getRoute(), $title, true, array(), 15, null, 'class="footer-blog-link"');
                }
            }
        }
    }
    ...
    protected function _prepareRelatedCategories($collection)
    {
        ...
        foreach ($categories as &$category) {
            ...
            $category['data'] = array('title' => $category['title'], 'url' => $this->getBlogUrl($category['identifier']));
        }
    ...
    {
        return Mage::app()->getRequest()->getModuleName() == Chestnut_Blog_Helper_Data::DEFAULT_ROOT;
    }
    ...
    protected function _prepareCollection()
    {
        if (!$this->getData('cached_collection')) {
            $collection = Mage::getModel('blog/blog')->getCollection()
                    ->addPresentFilter()
                    ->addEnableFilter(Chestnut_Blog_Model_Status::STATUS_ENABLED)
                    ->addStoreFilter()
                    ->joinComments()
