...
class Chestnut_Blog_Model_Mysql4_Category extends Mage_Core_Model_Mysql4_Abstract 
{
    ...
    protected function _afterSave(Mage_Core_Model_Abstract $object) 
    {
        ...
        $this->_getWriteAdapter()->delete($this->getTable('cat_parent'), $condition);
        ...
        if (!$object->getData('parent')) {
            $parents = array();
            $parents['cat_id'] = $object->getId();
            $parents['parent_id'] = 0;
            $this->_getWriteAdapter()->insert($this->getTable('cat_parent'), $parents);
        } else {
            foreach ((array) $object->getData('parent') as $parent) {
                $parents = array();
                $parents['cat_id'] = $object->getId();
